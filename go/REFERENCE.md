# BranchCohort Golang SDK Reference

Complete API reference for the BranchCohort Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *BranchCohortSDK`
...
#### `Analytics(data map[string]any) BranchCohortEntity`
...
| `filter` | `map[string]any` | No | "Keys are same as dimensions. |
...
```go
result, err := client.Analytics(nil).Load(map[string]any{"id": "analytics_id", "app_id": "app_id", "format": "format"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
