...
boru-driven command-line client **and** interactive REPL for the BranchCohort
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branch-cohort-cli)
make build

# 2. See usage (words, entities, env vars)
./branch-cohort-cli --help

# 3. Provide credentials once, via the environment
export BRANCH_COHORT_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./branch-cohort-cli load 1 analytics            # {id:1} shorthand
./branch-cohort-cli load '{id:1}' analytics       # explicit match map

# 5. Override the API base URL for a single call
BRANCH_COHORT_BASE=https://api.example.com ./branch-cohort-cli load 1 analytics

# 6. No arguments -> interactive REPL
./branch-cohort-cli
branch-cohort> load 1 analytics
branch-cohort> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branch-cohort-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/branch-cohort-cli load 1 analytics
   ```
...
```sh
./branch-cohort-cli load 1 analytics          # scalar shorthand for {id:1}
./branch-cohort-cli load '{id:1}' analytics     # explicit match map
```
...
./branch-cohort-cli load 1 analytics
```
...
Run with no arguments to open a REPL (prompt `branch-cohort>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/branch-cohort-cli`. |
...
(`./branch-cohort-cli <expr>`) and interactively (the REPL), and expressions compose the
