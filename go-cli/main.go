// Command BranchCohort-cli is a CLI + REPL for the BranchCohort SDK,
...
const prompt = "branch-cohort"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "analytics"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BRANCH_COHORT_APIKEY (api key), BRANCH_COHORT_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BRANCH_COHORT_APIKEY carries the API key and
	// BRANCH_COHORT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BRANCH_COHORT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BRANCH_COHORT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBranchCohortSDK(opts)
