...
// BranchCohort SDK retry feature
...
class BranchCohortRetryFeature extends BranchCohortBaseFeature
{
    ...
    public function init(BranchCohortContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BranchCohortContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BranchCohortContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BranchCohortContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
