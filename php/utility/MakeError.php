...
class BranchCohortMakeError
{
    public static function call(?BranchCohortContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BranchCohortContext([], null);
        }
        $op = $ctx->op ?? new BranchCohortOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BranchCohortResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BranchCohortError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BranchCohortError('', $msg, $ctx);
        ...
        if ($err instanceof BranchCohortError) {
