...
// BranchCohort SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class BranchCohortMakePoint
{
    public static function call(BranchCohortContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof BranchCohortError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = BranchCohortHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = BranchCohortHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
