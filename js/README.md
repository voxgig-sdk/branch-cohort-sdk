# BranchCohort JavaScript SDK



The JavaScript SDK for the BranchCohort API — an entity-oriented client with full async/await support.
...
```js
const { BranchCohortSDK } = require('@voxgig-sdk/branch-cohort-js')
...
```js
const analytics = await client.Analytics().load({ id: 'analytics_id', app_id: 'example_app_id', format: 'example_format' })
...
| `filter` | `Object` | "Keys are same as dimensions. |
...
branch-cohort/
├── src/
│   ├── BranchCohortSDK.js        # Main SDK class
