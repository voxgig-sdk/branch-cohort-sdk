...
class FtCtrl extends BranchCohortControl
{
...
class FtClient
{
    ...
    private BranchCohortUtility $utility;

    public function __construct(BranchCohortUtility $utility, array $options)
    {
    ...
    public function get_utility(): BranchCohortUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BranchCohortContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new BranchCohortUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (BranchCohortContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BranchCohortConfig::make_feature($name);
    ...
    private static function buildUrl(BranchCohortSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BranchCohortError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BranchCohortResponse($fetched);
        }
    ...
    private function fail(BranchCohortContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BranchCohortContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
