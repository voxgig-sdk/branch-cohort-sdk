...
// BranchCohort SDK pipeline test
//
...
{
    private static function utility(): BranchCohortUtility
    {
        return new BranchCohortUtility();
    }
    ...
    private static function ctx(array $over = []): BranchCohortContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BranchCohortContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BranchCohortOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BranchCohortError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BranchCohortOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BranchCohortMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BranchCohortOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BranchCohortMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BranchCohortOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BranchCohortMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BranchCohortSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BranchCohortMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BranchCohortResponse([]),
            'result' => new BranchCohortResult([]),
        ]);
        [, $err] = BranchCohortMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new BranchCohortSpec([]),
            'response' => null,
            'result' => new BranchCohortResult([]),
        ]);
        [, $err] = BranchCohortMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BranchCohortSpec(['step' => 's']),
            'response' => new BranchCohortResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BranchCohortResult([]),
        ]);
        [, $err] = BranchCohortMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BranchCohortSpec(['step' => 's']),
            'response' => new BranchCohortResponse(self::resp(200, ['v' => 1])),
            'result' => new BranchCohortResult([]),
        ]);
        [, $err] = BranchCohortMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BranchCohortSpec(['step' => 's']),
            'response' => new BranchCohortResponse(self::resp(200, ['v' => 2])),
            'result' => new BranchCohortResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BranchCohortMakeResponse::call($ctx);
    ...
    {
        $preset = new BranchCohortResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BranchCohortSpec([]),
            'response' => new BranchCohortResponse([]),
            'result' => new BranchCohortResult([]),
        ...
        [$got, $err] = BranchCohortMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BranchCohortResult([])]);
        [, $err] = BranchCohortMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BranchCohortSpec([]), 'result' => null]);
        [, $err] = BranchCohortMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BranchCohortSpec(['step' => 's']),
            'result' => new BranchCohortResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BranchCohortOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BranchCohortMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BranchCohortSpec(['step' => 's']),
            'result' => new BranchCohortResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BranchCohortOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BranchCohortMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BranchCohortResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BranchCohortSpec([]),
            'result' => new BranchCohortResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BranchCohortMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BranchCohortMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BranchCohortError('boom', 'boom');
        $utility->fetcher = function (BranchCohortContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BranchCohortSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BranchCohortMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BranchCohortContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BranchCohortMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BranchCohortResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        BranchCohortMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BranchCohortContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BranchCohortSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BranchCohortMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BranchCohortResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BranchCohortSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BranchCohortMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BranchCohortMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BranchCohortMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BranchCohortResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BranchCohortDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BranchCohortResult(['ok' => false])]);
        $this->expectException(BranchCohortError::class);
        BranchCohortDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BranchCohortResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BranchCohortMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        BranchCohortMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BranchCohortBaseFeature();
        $b = new BranchCohortBaseFeature();
        BranchCohortFeatureAdd::call($ctx, $a);
        BranchCohortFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BranchCohortBaseFeature
    {
        $f = new BranchCohortBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BranchCohortFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BranchCohortContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BranchCohortSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BranchCohortPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        BranchCohortPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BranchCohortResponse(['headers' => null]),
            'result' => new BranchCohortResult([]),
        ]);
        BranchCohortResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        BranchCohortResultBody::call($ctx);
