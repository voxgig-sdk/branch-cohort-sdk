# BranchCohort Lua SDK Reference

Complete API reference for the BranchCohort Lua SDK.
...
| `filter` | `table` | No | "Keys are same as dimensions. |
...
| `measures` | `table` | Yes | The cohort measures to return. |
...
```lua
local result, err = client:Analytics():load({ id = "analytics_id", app_id = "app_id", format = "format" })
```
