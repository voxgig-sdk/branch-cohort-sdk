# BranchCohort Lua SDK



The Lua SDK for the BranchCohort API — an entity-oriented client using Lua conventions.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/branch-cohort-sdk/releases)),
...
```lua
local analytics, err = client:Analytics():load({ id = "example_id", app_id = "example_app_id", format = "example_format" })
...
```lua
-- Create
local created, err = client:Analytics():create({ app_id = "example_app_id", format = "example_format", limit = 1, data_source = "example_data_source", end_date = "example_end_date", granularity_band_count = 1, measures = {}, start_date = "example_start_date" })
if err then error(err) end
...
```lua
local analytics, err = client:Analytics():load({ id = "example_id", app_id = "example", format = "example" })
if err then error(err) end
```
...
local result, err = client:Analytics():load({ id = "test01", app_id = "example", format = "example" })
...
| `filter` | `table` | "Keys are same as dimensions. |
...
| `measures` | `table` | The cohort measures to return. |
...
```lua
local analytics, err = client:Analytics():load({ id = "analytics_id", app_id = "app_id", format = "format" })
```
...
local analytics, err = client:Analytics():create({
  app_id = "example_app_id", -- string
  format = "example_format", -- string
  limit = 1, -- number
  data_source = "example_data_source", -- string
  end_date = "example_end_date", -- string
  granularity_band_count = 1, -- number
  measures = {}, -- table
...
```
lua/
├── branch-cohort_sdk.lua    -- Main SDK module
...
The main module (`branch-cohort_sdk`) exports the SDK constructor
