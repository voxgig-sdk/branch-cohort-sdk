...
[MCP](https://modelcontextprotocol.io) server exposing the BranchCohort SDK as
two agent tools — `branch-cohort_list` and `branch-cohort_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/branch-cohort-mcp)
make build

# 2. Provide credentials via the environment
export BRANCH_COHORT_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user branch-cohort \
  -- /absolute/path/to/branch-cohort-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./branch-cohort-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// branch-cohort_list: first page of records
{ "entity": "analytics" }
{ "entity": "analytics", "query": { } }

// branch-cohort_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/branch-cohort-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user branch-cohort \
     -- "$PWD"/dist/*/branch-cohort-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `branch-cohort_list` and `branch-cohort_load` tools now appear
   in new sessions. Ask the agent to *"list analytics using branch-cohort"*
   and it calls `branch-cohort_list` with `{"entity":"analytics"}`.
...
```sh
export BRANCH_COHORT_APIKEY=sk_live_xxx            # API key
export BRANCH_COHORT_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `branch-cohort_list` tool
...
### Call the `branch-cohort_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `branch-cohort_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `branch-cohort_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BRANCH_COHORT_APIKEY` | API key sent with every request. |
| `BRANCH_COHORT_BASE` | Optional override of the API base URL. |
...
```sh
./branch-cohort-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"branch-cohort_load","arguments":{"entity":"analytics","query":{"id":1}}}}'
```
