...
const __1 = require("..");
const SDK_NAME = 'BranchCohortSDK';
...
const TEST_SEED = { "entity": { "analytics": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+BranchCohortSDK\b/.test(code) ||
        /\bBranchCohortSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BranchCohortSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BranchCohortSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
