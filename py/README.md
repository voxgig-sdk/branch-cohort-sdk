# BranchCohort Python SDK



The Python SDK for the BranchCohort API — an entity-oriented client following Pythonic conventions.
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/branch-cohort-sdk/releases)) or
...
```python
try:
    analytics = client.Analytics().load({"id": "example_id", "app_id": "example_app_id", "format": "example_format"})
    print(analytics)
...
created = client.Analytics().create({"app_id": "example_app_id", "format": "example_format", "limit": 1, "data_source": "example_data_source", "end_date": "example_end_date", "granularity_band_count": 1, "measures": [], "start_date": "example_start_date"})
...
```python
try:
    analytics = client.Analytics().load({"id": "example_id", "app_id": "example", "format": "example"})
    print(analytics)
except Exception as err:
    print(f"load failed: {err}")
```
...
analytics = client.Analytics().load({"id": "test01", "app_id": "example", "format": "example"})
...
client = BranchCohortSDK(options)
```
...
```python
client = BranchCohortSDK.test(testopts, sdkopts)
```
...
| --- | --- | --- |
| `code` | `str` | HTTP code representing the outcome of the status request. |
...
| `data_source` | `str` | A string value representing the cohort type |
...
| `error_message` | `str` | Error message if the query failed |
| `filter` | `dict` | "Keys are same as dimensions. |
...
| `id` | `str` |  |
| `job_id` | `str` | Unique identifier used to retrieve job status and data. |
| `measures` | `list` | The cohort measures to return. |
| `ordered` | `str` | Order of response based on ordered_by value. |
| `ordered_by` | `str` | The dimension used for sorting |
| `per_user` | `bool` | If true, divide each band value by the user count. |
| `response_url` | `str` | S3 url for downloading the response data. |
...
| `status_url` | `str` | More information about the subscription's status. |
| `unique` | `bool` | Whether or not to return unique values. |

#### Example: Load

```python
analytics = client.Analytics().load({"id": "analytics_id", "app_id": "app_id", "format": "format"})
```
...
analytics = client.Analytics().create({
    ...
    "format": "example_format",  # str
    "limit": 1,  # int
    "data_source": "example_data_source",  # str
    "end_date": "example_end_date",  # str
    "granularity_band_count": 1,  # int
    "measures": [],  # list
...
```
py/
├── branchcohort_sdk.py         -- Main SDK module
...
The main module (`branchcohort_sdk`) exports the SDK class.
...
analytics.load({"id": "example_id", "app_id": "example", "format": "example"})
