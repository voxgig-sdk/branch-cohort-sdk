# BranchCohort SDK test runner

from __future__ import annotations
import os
import json

from branchcohort_sdk.utility.voxgig_struct import voxgig_struct as vs


class BranchCohortTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BranchCohortTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BranchCohortTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BranchCohortTestRunner.getenv("BRANCH_COHORT_TEST_LIVE")
        override = BranchCohortTestRunner.getenv("BRANCH_COHORT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BranchCohortTestRunner.getenv(key)
        ...
        explain = BranchCohortTestRunner.getenv("BRANCH_COHORT_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BRANCH_COHORT_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BranchCohortTestRunner._test_control is not None:
            return BranchCohortTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BranchCohortTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BranchCohortTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BranchCohortTestRunner.load_test_control()
...
def load_env_local():
    BranchCohortTestRunner.load_env_local()


def env_override(m):
    return BranchCohortTestRunner.env_override(m)


def entity_data(v):
    return BranchCohortTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BranchCohortTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BranchCohortTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BranchCohortTestRunner.load_test_control()


def live_client_options():
    return BranchCohortTestRunner.live_client_options()


def live_delay_ms():
    return BranchCohortTestRunner.live_delay_ms()
